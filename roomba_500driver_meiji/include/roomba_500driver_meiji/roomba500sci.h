...
#ifndef ROOMBA_500DRIVER_MEIJI__ROOMBA500SCI_H_
#define ROOMBA_500DRIVER_MEIJI__ROOMBA500SCI_H_
...
public:
  ...
  {
    const int usec = sec * 1000000;
    usleep(usec);
  }
};

const float COMMAND_WAIT = 0.01;  // sec, this time is for Roomba 500 series
const int16_t DEFAULT_VELOCITY = 200;  // mm/s

// DRIVE Special codes
const int16_t STRAIGHT_RADIUS = 0x8000;
const int16_t TURN_CLOCK = -1;
const int16_t TURN_CNT_CLOCK = 1;
...
protected:
  ...
  void packetToStruct(
    roomba_500driver_meiji::msg::Roomba500State & ret,
...
public:
  ...
  explicit roombaSci(int baud = B19200, const char * dev = "/dev/ttyUSB0");
  ~roombaSci();
  ...
  int16_t velToPWMRight(float velocity);
  int16_t velToPWMLeft(float velocity);
  float velToPWM(float velocity);

  int sendOPCODE(roombaSci::OPCODE);
  int getSensors();
  int getSensors(roomba_500driver_meiji::msg::Roomba500State & sensor);
...
#endif  // ROOMBA_500DRIVER_MEIJI__ROOMBA500SCI_H_
