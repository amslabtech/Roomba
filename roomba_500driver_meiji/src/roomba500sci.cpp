...
{
  const int16_t right = 1000 * (velocity + 0.5 * 0.235 * yawrate);
  const int16_t left = 1000 * (velocity - 0.5 * 0.235 * yawrate);
    ...
    const unsigned char* pack)
{
  ...
  ret.wall_signal = (uint16_t)((pack[26] << 8) | pack[27]);
  ...
  ret.dirt_detect = (uint16_t)(pack[8]);

  ret.cliff.left_signal = (uint16_t)((pack[28] << 8) | pack[29]);
  ret.cliff.front_left_signal = (uint16_t)((pack[30] << 8) | pack[31]);
  ret.cliff.front_right_signal = (uint16_t)((pack[32] << 8) | pack[33]);
  ret.cliff.right_signal = (uint16_t)((pack[34] << 8) | pack[35]);
  ...
  ret.distance = (int16_t)((pack[12] << 8) | pack[13]);
  ret.angle = (int16_t)((pack[14] << 8) | pack[15]);
  ret.requested_wheel_velocity.right = (int16_t)((pack[48] << 8) | pack[49]);
  ret.requested_wheel_velocity.left = (int16_t)((pack[50] << 8) | pack[51]);

  ret.dirt_detect = (uint16_t)(pack[39]);
  ret.open_interface_mode = (uint16_t)(pack[40]);

  ret.song.number = (uint16_t)(pack[41]);
  ret.song.playing = (uint16_t)(pack[42]);

  ret.oi_stream_num_packets = (uint16_t)(pack[43]);

  ret.requested_velocity = (int16_t)((pack[44] << 8) | pack[45]);
  ret.requested_radius = (int16_t)((pack[46] << 8) | pack[47]);
  ret.encoder_counts.left = (uint16_t)((pack[52] << 8) | pack[53]);
  ret.encoder_counts.right = (uint16_t)((pack[54] << 8) | pack[55]);

  ret.light_bumper.bumper = (uint16_t)(pack[56]);
  ret.light_bumper.left = (uint16_t)((pack[57] << 8) | pack[58]);

  ret.light_bumper.front_left = (uint16_t)((pack[59] << 8) | pack[60]);
  ret.light_bumper.center_left = (uint16_t)((pack[61] << 8) | pack[62]);
  ret.light_bumper.center_right = (uint16_t)((pack[63] << 8) | pack[64]);
  ret.light_bumper.front_right = (uint16_t)((pack[65] << 8) | pack[66]);
  ret.light_bumper.right = (uint16_t)((pack[67] << 8) | pack[68]);

  ret.opcode.left = (uint16_t)(pack[69]);
  ret.opcode.right = (uint16_t)(pack[70]);

  ret.stasis = static_cast<bool>(0x01 & (pack[79]));

  if (std::abs(static_cast<int>(ret.encoder_counts.right) - static_cast<int>(enc_count_r_)) >= 60000)
  {
  ...
  if (std::abs(static_cast<int>(ret.encoder_counts.left) - static_cast<int>(enc_count_l_)) >= 60000)
  {
